...
export default function EditServiceByForm() {
    ...
    await post(URL.URL_EDIT_SERVICE + params.id, { ...data })
      .then((res) => {
        if (res.data.status === 0) {
          ...
          message.success(
            `Service ${service.serviceName} chỉnh sửa thành công!`
          );
  ...
  return (
    <>
      ...
      >
        <p>Bạn có chắc chắn muốn sửa service?</p>
      </Modal>
      <Form
        {...layout}
        className="!pr-[100px]"
      ...
      >
        ...
        <EmailInput
          ...
          initValue={service.authorizedPerson}
        ></EmailInput>
        ...
        <Form.Item
          ...
          initialValue={Object.keys(service.requirement.infrastructure).filter(
            (value) => {
